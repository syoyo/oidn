...
namespace oidn {
                                                  ...
                                                  void* data)
  {
    ...
    memory::primitive_desc primDesc(dims, memory::data_type::f32, format);
    if (data == nullptr)
                                                 ...
                                                 size_t srcOffset)
  {
    ...
    memory::primitive_desc primDesc(dims, memory::data_type::f32, BlockedFormat<K>::nChwKc);
    float* srcPtr = src->get_data() + srcOffset;
  ...
  void Network<K>::zeroTensor(const std::shared_ptr<memory>& dst)
  {
    ...
    memset(dst->get_data(), 0, getTensorSize(dst)*sizeof(float));
  }
                                            ...
                                            bool relu)
  {
    ...
    auto weightsDesc = memory::primitive_desc({ weightsPadDims }, memory::data_type::f32, memory::format::any);

    auto convAlgo = nnp_convolution_algorithm_direct;
#if 0 // TODO(LTE):
